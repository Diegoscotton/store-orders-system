...
import { getMasterStores, toggleStoreActive, extendStoreTrial } from '@/services/masterService'
...
import { Store, Eye, ExternalLink, Power, PowerOff, Clock, Package, ShoppingCart, Calendar, User, Phone, Mail } from 'lucide-react'
...
export default function MasterStoresPage() {
  ...
  async function handleToggle(store: StoreWithOwner) {
    ...
    try {
      await toggleStoreActive(store.id, newState)
      toast({ type: 'success', title: `Loja ${newState ? 'ativada' : 'desativada'}` })
      await loadStores()
    } catch {
      toast({ type: 'error', title: `Erro ao ${action} loja` })
    }
  }
                  ...
                  <tr key={store.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      {isExpired ? (
                        ...
                        <Badge variant="warning">{daysLeft} dia{daysLeft !== 1 ? 's' : ''}</Badge>
                      ) : (
